...
function SHAMapTreeNodeInner(depth) {
  ...
  this.leaves = {}
  this.type = SHAMapTreeNode.TYPE_INNER
  ...
  if (existing_node) {
    ...
    if (existing_node instanceof SHAMapTreeNodeInnerV2) {
      if (existing_node.has_common_prefix(tag)) {
        ...
        const new_inner_node = new SHAMapTreeNodeInnerV2(new_depth)
      ...
      // Turn it into an inner node
      const new_inner_node = new SHAMapTreeNodeInnerV2(0)
...
SHAMapTreeNodeInnerV2.prototype.hash = function () {
  ...
  const prefix = hashprefixes.HASH_INNER_NODE_V2.toString(16)
...
SHAMapTreeNodeLeaf.prototype.hash = function () {
  switch (this.type) {
    ...
    case SHAMapTreeNode.TYPE_TRANSACTION_NM:
      const txIDPrefix = hashprefixes.HASH_TX_ID.toString(16)
      return hash(txIDPrefix + this.data)
    case SHAMapTreeNode.TYPE_TRANSACTION_MD:
      const txNodePrefix = hashprefixes.HASH_TX_NODE.toString(16)
